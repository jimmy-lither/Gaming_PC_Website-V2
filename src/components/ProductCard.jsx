...
export default function ProductCard({ product, isComponent = false }) {
  ...
  const formatSpecs = (specs) => {
    ...
    return Object.entries(specs).slice(0, 4).map(([key, value]) => (
      <div key={key}>
        <span className="text-gray-500 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}:</span> {value}
      </div>
    ));
  };
  ...
  if (isComponent) {
    return (
      <motion.div
        ...
        className="group relative bg-brand-surface rounded-lg overflow-hidden border border-white/10 hover:border-brand-cyan/50 transition-all hover:shadow-lg hover:shadow-brand-cyan/10"
      >
        <Link to="#">
        ...
        <div className="p-4">
          <p className="text-xs text-brand-cyan mb-1 uppercase tracking-wide">{product.brand}</p>
          <Link to="#">
